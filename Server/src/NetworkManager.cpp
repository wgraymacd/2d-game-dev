    ...
    while (enet_host_service(m_server, &event, waitTime) > 0) // enet_host_service polls for network events (&event: stores the next network event if one exists; 0: doesn't wait for new events)
    {
        ...
        case ENET_EVENT_TYPE_RECEIVE:
            ...
            case VELOCITY:
                ...
                break;
            case SPAWN:
                std::cout << "Received: " << std::to_string(m_data->dataType) << ", " << m_data->id << "\n";
                printf("Creating new net entity.\n");
                m_data->data.netId = createNetEntity();
                sendData(*m_data);
...
// void NetworkManager::processPosition()
// {
...
NetEntityID NetworkManager::createNetEntity()
{
